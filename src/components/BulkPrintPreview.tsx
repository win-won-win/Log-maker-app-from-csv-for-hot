...
interface BulkPrintPreviewProps {
  records: ServiceRecord[];
  onClose: () => void;
}

export function BulkPrintPreview({ records, onClose }: BulkPrintPreviewProps) {
  ...
  const handlePrint = async () => {
    try {
      // 各記録に対して印刷時間のみを生成（記録作成日時は既存のものを使用）
      const recordsWithPrintTime = records.map((record, index) => {
        const times = generateTimesForRecord(record);
        const printTime = new Date(times.printDateTime);
        printTime.setSeconds(printTime.getSeconds() + index * 2); // 2秒ずつずらす
        return {
          ...record,
          print_datetime: printTime.toISOString()
          // record_created_atは既存の値をそのまま使用
        };
      });

      // データベースの印刷時間を一括更新
      const updatePromises = recordsWithPrintTime.map(record =>
        supabase
          .from('service_records')
          .update({ print_datetime: record.print_datetime })
          .eq('id', record.id)
      );
      ...
      if (errors.length > 0) {
        console.error('印刷時間更新エラー:', errors);
        alert(`${errors.length}件の印刷時間更新に失敗しました`);
        return;
      }

      // 記録データをLocalStorageに保存（印刷時間を含む）
      localStorage.setItem('printRecords', JSON.stringify(recordsWithPrintTime));
      ...
      console.error('一括印刷処理エラー:', error);
      alert('一括印刷処理中にエラーが発生しました');
    }
  };
  ...
  const renderRecord = (record: ServiceRecord, index: number) => {
    const details = record.service_details || {};

    return (
      <div key={record.id} className={`print-page ${index > 0 ? 'page-break' : ''}`}>
        {/* ヘッダー部分 */}
        ...
        <div className="border border-black mb-2">
          <table className="w-full text-xs compact-table">
            <tbody>
              <tr className="table-row-expanded" style={{height: '50px'}}>
                <td className="border-r border-black p-2 bg-gray-100 font-medium" style={{width: '60px'}}>利用者名</td>
                <td className="border-r border-black p-2" style={{width: '80px'}}>{record.user_name}様</td>
                <td className="border-r border-black p-2 bg-gray-100 font-medium" style={{width: '90px'}}>サービス実施日</td>
                <td className="border-r border-black p-2" style={{width: '100px'}}>{formatDate(record.service_date)}</td>
                <td className="border-r border-black p-2 bg-gray-100 font-medium" style={{width: '90px'}}>提供ヘルパー名</td>
                <td className="border-r border-black p-2" style={{width: '70px'}}>{record.staff_name}</td>
                <td className="border-r border-black p-2 bg-gray-100 font-medium" style={{width: '90px'}}>利用者確認印</td>
                <td className="p-2 text-center" style={{width: '60px'}}></td>
              </tr>
              <tr className="border-t border-black table-row-expanded" style={{height: '50px'}}>
                <td className="border-r border-black p-2 bg-gray-100 font-medium" style={{width: '72px'}}>サービス時間</td>
                <td className="border-r border-black p-2" style={{width: '140px'}}>{formatTime(record.start_time)}～{formatTime(record.end_time)}</td>
                <td className="border-r border-black p-2 bg-gray-100 font-medium" style={{width: '90px'}}>サービス種類</td>
                <td className="border-r border-black p-2" style={{width: '100px'}}>{getServiceTypeDisplayName(record)}</td>
                <td className="border-r border-black p-2 bg-gray-100 font-medium" style={{width: '90px'}}>記録作成日時</td>
                <td className="border-r border-black p-2" style={{width: '150px'}}>{record.record_created_at ? new Date(record.record_created_at).toLocaleString('ja-JP') : ''}</td>
                <td className="border-r border-black p-2 bg-gray-100 font-medium" style={{width: '90px'}}>責任者確認印</td>
                <td className="p-2 text-center" style={{width: '60px'}}></td>
              </tr>
            </tbody>
          </table>
